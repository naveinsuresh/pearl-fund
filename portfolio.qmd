...
ggplot(aes(x = date, y = MultiplePortfolioMonthlyReturns, color = portfolio)) +
  ...
  geom_hline(aes(yintercept = mean(filter(multiple_portfolio_returns_monthly, portfolio == 1)$MultiplePortfolioMonthlyReturns)))
```
...
```{r}
statstss = spy_combined_table_of_returns %>% tq_performance(Ra = AggregateMonthlyReturns, Rb = NULL, performance_fun = table.Stats)
statstss[, c("GeometricMean", "Stdev")] %>% mutate( AnnualizedSD = Stdev * sqrt(12), Annualizedreturn = (1+ GeometricMean)^(12) - 1)
```
...
# pspec = add.objective(portfolio = pspec, type='risk', name="var", risk_aversion=7)
...
pspec <- add.constraint(portfolio=pspec, type="box", min=0.05, max=0.95)
...
opt_maxret <- optimize.portfolio(R=stock_returns_monthly_wide, portfolio=pspec, optimize_method="ROI", trace=TRUE)
...
```{r}
weights3 = optimized_vector_of_weights
weights3

```

```{r}
# Given monthly return
monthly_return <- 0.0306
