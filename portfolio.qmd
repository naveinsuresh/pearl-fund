...
Multiple Portfolio Aggregation

```{r}
map_of_weights <- tibble(
...
stock_returns_monthly %>%
    tq_portfolio(assets_col  = symbol, 
                 returns_col = MonthlyReturns, 
                 weights     = map_of_weights, 
                 col_rename  = "MappedMonthlyReturns")
```
...
multiple_stock_returns_monthly <- stock_returns_monthly %>%
    tq_repeat_df(n = 2)
...
weights_portfolio_table <- tibble(stock_tickers) %>%
    tq_repeat_df(n = 2) %>%
    bind_cols(tibble(weights)) %>%
    group_by(portfolio)
weights_portfolio_table
```
...
Statistical Testing Multiple Portfolios
