...
stock_returns_monthly = stock_returns_monthly_raw %>% group_by(symbol) %>% tq_transmute(select = adjusted, mutate_fun = periodReturn, period = "monthly", col_rename = "Monthly Returns")
...
```{r}
stock_returns_monthly %>% ggplot(aes(date, `Monthly Returns`, color=symbol)) + geom_line()
```

```{r}
stock_returns_monthly %>% ggplot(aes(date, `Monthly Returns`, color=symbol)) + 
  geom_line() +
  facet_wrap(~ symbol)
```
...
spy_returns_monthly = tq_transmute(spy_returns_monthly_raw, select = adjusted, mutate_fun = periodReturn, period = "monthly", col_rename = "SPY Monthly Returns")
...
nasdaq_returns_monthly = tq_transmute(nasdaq_returns_monthly_raw, select = adjusted, mutate_fun = periodReturn, period = "monthly", col_rename = "NASDAQ Monthly Returns")
...
aggregate_portfolio_monthly_returns = stock_returns_monthly %>% tq_portfolio(assets_col = symbol, returns_col = `Monthly Returns`, weights = weights1, col_rename = "Aggregate Monthly Returns")
...
```{r}
aggregate_portfolio_monthly_returns %>% ggplot(aes(x = date, y = `Aggregate Monthly Returns`)) + geom_bar(stat = "identity", fill = palette_light()[[1]]) + labs(title = "Pearl Fund Portfolio Returns", subtitle = "20% AMD, 20% INTC,20% NVDA, 20% SMCI, and 20% TSM", caption = "Shows an above-zero trend indicating long-term positive returns", x = "", y = "Monthly Returns") + geom_smooth(method = "lm") + theme_tq() + scale_color_tq() + scale_y_continuous(labels = scales::percent)
```

```{r}
aggregate_portfolio_monthly_returns %>% ggplot(aes(x = date, y = `Aggregate Monthly Returns`)) + geom_point() + labs(title = "Pearl Fund Portfolio Returns", subtitle = "20% AMD, 20% INTC,20% NVDA, 20% SMCI, and 20% TSM", caption = "Shows an above-zero trend indicating long-term positive returns", x = "", y = "Monthly Returns") + 
...
ggplot(three_fund_combined_table_of_returns, aes(x = date)) +
  geom_line(aes(y = `Aggregate Monthly Returns`, color = "Aggregate"), linetype = "solid", size = 1) +
  geom_line(aes(y = `SPY Monthly Returns`, color = "SPY"), linetype = "solid", size = 1) +
  geom_line(aes(y = `NASDAQ Monthly Returns`, color = "NASDAQ"), linetype = "solid", size = 1) +
...
#statisticsA = spy_combined_table_of_returns %>% tq_performance(Ra = `Aggregate Monthly Returns`, Rb = `SPY Monthly Returns`, performance_fun = table.CAPM)

# Pearl Fund against NASDAQ
statisticsB = nasdaq_combined_table_of_returns %>% tq_performance(Ra = `Aggregate Monthly Returns`, Rb = `NASDAQ Monthly Returns`, performance_fun = table.CAPM)
...
We now form a combined vector called *weights* which concactenates both our portfolio desired weights together.
...
```{r}
combined_benchmark_multiple_portfolio %>%
    tq_performance(Ra = MultiplePortfolioMonthlyReturns, Rb = `SPY Monthly Returns`, performance_fun = table.CAPM)
```
...
Stock_Statistics = stock_returns_monthly %>% group_by(symbol)  %>% summarise(Historic_Expected_Returns = mean(`Monthly Returns`), Standard_Deviation = sd(`Monthly Returns`), Variance = VAR(`Monthly Returns`), Count = n())
Stock_Statistics 
```
...
how do we make this more organised and efficient

```{r}
new_table = data.frame( symbol = c("AMD"), `Monthly Excess Returns` = filter(stock_returns_monthly, symbol == "AMD")$`Monthly Returns` - filter(Stock_Statistics, symbol == "AMD")$Expected_Returns)
new_table 
```

```{r}
new_table1 = data.frame( symbol = c("INTC"), `Monthly Excess Returns` = filter(stock_returns_monthly, symbol == "INTC")$`Monthly Returns` - filter(Stock_Statistics, symbol == "INTC")$Expected_Returns)
new_table1
```

```{r}
new_table2 = data.frame( symbol = c("AMD", "INTC", "NVDA", "QCOM", "TSM"), `Monthly Excess Returns` = stock_returns_monthly$`Monthly Returns` - Stock_Statistics$Expected_Returns)
new_table2
```

```{r}
new_table3 = data.frame( symbol = c("NVDA"), `Monthly Excess Returns` = filter(stock_returns_monthly, symbol == "NVDA")$`Monthly Returns` - filter(Stock_Statistics, symbol == "NVDA")$Expected_Returns)
new_table3
```

```{r}
new_table4 = data.frame( symbol = c("QCOM"), `Monthly Excess Returns` = filter(stock_returns_monthly, symbol == "QCOM")$`Monthly Returns` - filter(Stock_Statistics, symbol == "QCOM")$Expected_Returns)
new_table4
```

```{r}
new_table5 = data.frame( symbol = c("TSM"), `Monthly Excess Returns` = filter(stock_returns_monthly, symbol == "TSM")$`Monthly Returns` - filter(Stock_Statistics, symbol == "TSM")$Expected_Returns)
new_table5
```

```{r}
TableX = rbind(new_table, new_table1, new_table3, new_table4, new_table5)
TableX
```
...
n <- 240
tX <- t(X)
var_cov_mat <- (tX %*% X) / (n - 1)

var_cov_mat
```

```{r}
returns_table <- stock_returns_monthly %>% left_join(Stock_Statistics, by = "symbol") %>% mutate(Excess_Returns = `Monthly Returns` - Historic_Expected_Returns)
