...
```{r}
Excess_Return = Stock_Statistics$Expected_Returns - (latest_ten_year_treasury_yield/100)
Excess_Return
```
...
```{r}
filter(stock_returns_monthly, symbol == "AMD")$`Monthly Returns` - filter(Stock_Statistics, symbol == "AMD")$Expected_Returns
```
