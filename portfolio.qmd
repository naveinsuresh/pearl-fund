...
spy_returns_monthly = tq_transmute(spy_returns_monthly_raw, select = adjusted, mutate_fun = periodReturn, period = "monthly", col_rename = "SPY_Monthly_Returns")
...
nasdaq_returns_monthly = tq_transmute(nasdaq_returns_monthly_raw, select = adjusted, mutate_fun = periodReturn, period = "monthly", col_rename = "NASDAQ_Monthly_Returns")
...
aggregate_portfolio_monthly_returns = stock_returns_monthly %>% tq_portfolio(assets_col = symbol, returns_col = `Monthly Returns`, weights = weights1, col_rename = "Aggregate_Monthly_Returns")
...
```{r}
aggregate_portfolio_monthly_returns %>% ggplot(aes(x = date, y = `Aggregate_Monthly_Returns`)) + geom_bar(stat = "identity", fill = palette_light()[[1]]) + labs(title = "Pearl Fund Portfolio Returns", subtitle = "20% AMD, 20% INTC,20% NVDA, 20% SMCI, and 20% TSM", caption = "Shows an above-zero trend indicating long-term positive returns", x = "", y = "Monthly Returns") + geom_smooth(method = "lm") + theme_tq() + scale_color_tq() + scale_y_continuous(labels = scales::percent)
```
...
ggplot(three_fund_combined_table_of_returns, aes(x = date)) +
  geom_line(aes(y = `Aggregate_Monthly_Returns`, color = "Aggregate"), linetype = "solid", size = 1) +
  geom_line(aes(y = `SPY_Monthly_Returns`, color = "SPY"), linetype = "solid", size = 1) +
  geom_line(aes(y = `NASDAQ_Monthly_Returns`, color = "NASDAQ"), linetype = "solid", size = 1) +
...
statisticsA = spy_combined_table_of_returns %>% tq_performance(Ra = `Aggregate_Monthly_Returns`, Rb = `SPY_Monthly_Returns`, performance_fun = table.CAPM)

# Pearl Fund against NASDAQ
statisticsB = nasdaq_combined_table_of_returns %>% tq_performance(Ra = `Aggregate_Monthly_Returns`, Rb = `NASDAQ_Monthly_Returns`, performance_fun = table.CAPM)
...
```{r}
combined_benchmark_multiple_portfolio %>%
    tq_performance(Ra = MultiplePortfolioMonthlyReturns, Rb = `SPY_Monthly_Returns`, performance_fun = table.CAPM)
```
...
```{r}
combined_benchmark_multiple_portfolio %>%
    tq_performance(Ra = MultiplePortfolioMonthlyReturns, Rb = NULL, performance_fun = SharpeRatio, Rf = 0.0427)
```
...
We start by calculating key statistics for each of our stocks in our portfolio, specifically the *Expected Return*, *Standard Deviation*, *Variance*, and *Total Count*.

```{r}
Stock_Statistics = stock_returns_monthly %>% group_by(symbol) %>% summarise(Expected_Returns = mean(`Monthly Returns`), Standard_Deviation = sd(`Monthly Returns`), Variance = VAR(`Monthly Returns`), Count = n())

Stock_Statistics 
```

Next, we move to calculate *Excess Monthly Returns* for each stock using the following formula by subtracting the columns of the Monthly Returns from the *Expected Returns* we calculated earlier.

```{r}
returns_table <- stock_returns_monthly %>% left_join(Stock_Statistics, by = "symbol") %>% mutate(Excess_Returns = `Monthly Returns` - Expected_Returns)

excess_monthly_returns_table <- returns_table %>% select(`symbol`, `date`, `Excess_Returns`)
excess_monthly_returns_table
```

The next step is to calculate the two essential matrices: *Covariance Matrix & Correlation Matrix* as utilized in efficient set mathematics.

**Key Questions:**

-   How do we ultimately incorporate the risk-free return in our excess portfolio returns and in our efficient frontier model?

-   Do stock returns, the ones we selected, follow a normal distribution, if so how do we prove/test?

    -   Jacques Bera test, Shapiro-Wilt Test?

-   Are there existing packages that compute these matrices and statistics for us in R and are there packages for the optimization part as well?

-   How do we interpret the CAPM statistics and the Sharpe Ratio statistics from the tidyquant package in our analysis?

-   How do we take this forward, trading strategies?, alpha generation?, forecasting?, etc...
