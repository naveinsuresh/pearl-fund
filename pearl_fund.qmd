...
We run a *Principal Component Analysis* calculation to display the groupings and correlations among the various assets in the pearl fund. The graph below displays the groups of closely related or correlated associations with certain stocks. Additionally, groups of stocks that are directly opposite to other groups (around 180 degrees) show stocks that are correlated in directly opposite directions (ex. MSFT/COST vs. RTX).
...
equally_weighted_portfolio_monthly_returns <- stock_returns_monthly_long %>%
    tq_portfolio(assets_col = symbol, 
                 returns_col= `MonthlyReturns`, 
...
ggplot(aggregate_portfolio_monthly_returns, aes(x = date, y = investment.growth)) +
...
```{r}
animation2 = ggplot() +
  geom_line(data = aggregate_portfolio_monthly_returns, aes(x = date, y = investment.growth), 
            linewidth = 2, color = "red") +
  geom_smooth(data = aggregate_portfolio_monthly_returns, aes(x = date, y = investment.growth), 
              method = "loess", color = "red", se = FALSE) +
  geom_line(data = equally_weighted_portfolio_monthly_returns, aes(x = date, y = investment.growth), 
            linewidth = 2, color = "blue") +
  geom_smooth(data = equally_weighted_portfolio_monthly_returns, aes(x = date, y = investment.growth), 
