...
This code below was an output of CHATGPT cause it said our code above was incorrect?? Not sure whats different but output is different

```{r}
excess_returns_table1 <- stock_returns_monthly_long %>%
  left_join(Stock_Statistics, by = "symbol") %>%
  mutate(Excess_Returns1 = MonthlyReturns - Historic_Expected_Returns) %>%
  select(symbol, date, Excess_Returns1)

# Order by date in descending order and display the most recent 5 monthly excess stock returns
excess_returns_table_decreasing1 <- excess_returns_table1 %>%
  arrange(desc(date)) %>%
  head(5)

excess_returns_table_decreasing1
```
...
# pearlfund = add.constraint(portfolio=pearlfund, type="return", return_target=0.002)
...
plot(efficient.frontier, c(1,3,4,7))
