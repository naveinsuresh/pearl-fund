...
In this project, our primary goal is to enhance the risk-adjusted returns of a carefully curated portfolio of stocks. The focus of our analysis is on a select 5-stock portfolio, nicknamed — "Pearl Fund." This portfolio comprises leading semiconductor companies in the market, namely:
...
```{r}
stock_tickers <- c("TLT", "MSFT", "AAPL", "COST", "F", "PEP", "JPM", "RTX", "PG", "CVX", "JNJ")
...
head(stock_returns_monthly_full_wide_decreasing, 5)
...
We repeat the above process for the 2nd batch of monthly stock returns (2019 -\> 2024)
...
head(stock_returns_monthly_test_wide_decreasing, 5)
```
...
```{r}
abcd = ggplot(Stock_Statistics, aes(x = Standard_Deviation, y = Historic_Expected_Returns, color = symbol)) +
geom_point(size = 5) +
...
head(excess_returns_table_decreasing,20)
```
...
\*correlogram, heatmaps for correlations

```{r}
# another plot
pca_df = as.data.frame(cor(d, pca$scores))
pca_df |>
...
pearlfund = add.constraint(portfolio=pearlfund, type="return", return_target=0.0153)
...
Here is another display of our efficient frontier using a different R frontier calculation package:
...
Finally, we plot these ETL values against our return values for our portfolios and are able to visualize the results.
...
[Industries to Focus On:]{.underline}
